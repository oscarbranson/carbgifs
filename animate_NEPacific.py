...
fname = f'animations/{savename}_{depth[-1]:.0f}-{depth[0]:.0f}m'
...
env0 = {k + '_in': v[-1] for k, v in env.items()}
...
pS = sax.scatter(env['S'][-1], depth[-1], zorder=2, color='k')
pT = tax.scatter(env['T'][-1], depth[-1], zorder=2, color='k')

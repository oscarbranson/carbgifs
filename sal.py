...
sw = cb.Csys(DIC=DIC.flat, TA=TA.flat, T_in=T, S_in=xS[0], P_in=P)
...
def animate_S(i):
    ...
    sw = cb.Csys(DIC=DIC.flat, TA=TA.flat, T_in=T, P_in=P, S_in=xS[i])
    ...
    ax.set_title(f'Sal = {xS[i] * 10:.1f}')


# make animation
anim  = animation.FuncAnimation(fig, animate_S, frames=nframes)
